...
(Note: The code below only runs if the model file does not exists. The training algorithm does not create perfectly identical word vector models, which can cause problems when re-running the vignette.)
...
if (!file.exists(paste0(model_file, ".bin"))) {
...
We want to narrow down this list again, filtering terms that occur only once. And we add a unique word_id by `add_word_id`.
```{r}
top %<>% filter(hits > 1) 
top %<>% add_word_id()
```
...
These 10 words cover various aspects of populist communication and are highly context-specific. However, this is not the place to dive into theoretical discussions.

Let's check the performance of the dictionary instead. On the training sample, this dictionary reaches an F1 of .56, which is not great, but OK.
```{r f1 on train}
combis_df$F1 %>% max()  
```
...
```
The short dictionary reaches a F1 of .55 in the test sample, which is almost par with the result for the train data. Drawing on this evaluation, we have some reason to assume that we avoided overfitting the dictionary to the training data. We may therefore assume that this measurement can at least approximately capture populist communication in our population dataset. It does not mean that this short dictionary would perform equally well in different contexts, e.g., in user-generated material, or in other timeframes.

For some special applications, you might be intereste how your DDR measurement perform for different subsets of your data, e.g., if you are using a translated corpus originating from various languages. F1 scores for grouped subsets of a text dataframe, can be obtained by `get_many_F1s_by_group`. 

In our case, we might be interested how the dictionaries perform for different political parties. Here, we demonstrate the function using the top 5% of short dictionaries. Since the populist communication in our annotated sample is indeed mainly concentrated in the parties AfD and Die Linke, we calculate the 3rd root of the product of the overall F1 score and the two party-specific F1-scores to pick the best performing, balanced dictionary:
```{r f1 by group}
...
```
As this decreased the performance in the test data, we stick with the dictionary picked before.

## Apply DDR

Now, we want to apply the best performing dictionary to the datasets. We apply it on both, the population data `tw_data` and the annotated data `tw_annot`, using the core function of this package `cossim2dict`. We fill in missing values, which occure sometimes if text cleaning empties the text field by the mean minus 1 SD: 
```{r applying ddr}
...
Now, lets inspect the top 3 populist Tweets in our population data:
```{r face validity 2}
tw_data %>% arrange(desc(pop_ddr)) %>% pull(full_text) %>% head(3)

```
These examples include Tweets from the AfD directed against 'genderism' and leftist ideology, directed against the EU, and directed against climate politics. Judging from face validity these Tweets are clearly populist.

Let's inspect the lower end of the spectrum:
```{r face validity}
tw_data %>% arrange(desc(pop_ddr)) %>% pull(full_text) %>% tail(3)
